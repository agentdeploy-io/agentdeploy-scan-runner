...
import {
  ...
  getSellerTemplates,
  DirectusForbiddenError,
} from "../services/directus.js";
import { logger } from "../logger.js";
import { redactSecrets, safeLogContext } from "../lib/redact.js";
import { RATING_TO_COLOR, SCAN_MINIMUM_RATING_FOR_DEPLOY } from "../constants.js";
...
// Type for validated scan request
...
async function processScanAsync(
  ...
  try {
    ...
    const categoryRatings: Record<string, ReturnType<typeof calculateCategoryRating>> = {};
    ...
    await updateScanJob(jobId, {
      ...
      rating_secrets: (categoryRatings.secrets?.rating ?? "A") as typeof result.overallRating,
      rating_prompt_injection: (categoryRatings.prompt_injection?.rating ?? "A") as typeof result.overallRating,
      rating_dependencies: (categoryRatings.dependencies?.rating ?? "A") as typeof result.overallRating,
      rating_permissions: (categoryRatings.permissions?.rating ?? "A") as typeof result.overallRating,
      rating_sast: (categoryRatings.sast?.rating ?? "A") as typeof result.overallRating,
    ...
    try {
      logger.info(
        { sellerId, worstRating, score: result.overallScore },
        "📝 Updating seller security fields after scan"
      );
      ...
      if (error instanceof DirectusForbiddenError) {
        ...
        logger.warn(
          { sellerId, templateId },
          "⚠️  Scan completed but seller security fields were not updated due to permission error"
        );
      } else {
        logger.error(
          { sellerId, templateId, jobId, error },
...
scanRoute.get("/scan/status/:jobId", authMiddleware, async (c) => {
  ...
  try {
    ...
    if (job.status === "pending") {
      ...
      response.metadata = job.metadata as Record<string, unknown> | undefined;
    ...
    return c.json(
      { error: { code: "STATUS_FETCH_FAILED", message: "Failed to fetch job status" } },
      500
    );
  }
});
