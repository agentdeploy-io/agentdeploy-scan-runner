import { SCAN_RATING_THRESHOLDS, RATING_TO_COLOR, SCAN_MINIMUM_RATING_FOR_DEPLOY, SCAN_RATING_ORDER, } from "../constants.js";
export function scoreToRating(score) {
    if (score >= SCAN_RATING_THRESHOLDS.A)
        return "A";
    if (score >= SCAN_RATING_THRESHOLDS.B)
        return "B";
    if (score >= SCAN_RATING_THRESHOLDS.C)
        return "C";
    if (score >= SCAN_RATING_THRESHOLDS.D)
        return "D";
    return "F";
}
export function calculateCategoryRating(findings) {
...
export function aggregateRatings(categoryRatings) {
    ...
    const overallScore = Math.round(Object.values(categoryRatings).reduce((sum, r) => sum + r.score, 0) /
        categories.length);
    return {
        ratings: categoryRatings,
        overallRating,
...
export function isRatingDeployable(rating) {
