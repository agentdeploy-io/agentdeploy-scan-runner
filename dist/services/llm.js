...
function normalizeNumericFindings(value) {
    const parsed = Number(value);
...
function normalizeLlmPayload(raw) {
    ...
    const categories = ["secrets", "prompt_injection", "dependencies", "permissions", "sast"];
 ...
 */
function orderProviders(providers, fallbackOrder) {
...
// ─── Main entry point ──────────────────────────────────────────────
export async function analyzeWithLLM(bundledCode) {
    const env = getEnv();
    ...
    const truncatedCode = bundledCode.length > 100_000
