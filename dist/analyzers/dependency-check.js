...
export function analyzeDependencies(files) {
    const findings = [];
    const packageJsonFile = files.find((f) => f.relativePath === "package.json" || f.relativePath.endsWith("/package.json"));
    if (packageJsonFile) {
    ...
    const mcpManifest = files.find((f) => f.relativePath === "mcp.json" ||
...
function checkPackageJson(file, findings) {
    let pkg;
    try {
...
function checkRequirementsTxt(file, findings) {
...
function checkMcpManifest(file, findings) {
    try {
...
function findDependencyLine(content, depName) {
