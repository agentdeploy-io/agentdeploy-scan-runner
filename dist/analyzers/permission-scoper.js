export function analyzePermissions(files) {
    const findings = [];
    const dockerfiles = files.filter((f) => f.relativePath === "Dockerfile" ||
    ...
    const k8sFiles = files.filter((f) => {
        const lower = f.relativePath.toLowerCase();
        return (lower.includes("deployment.yaml") ||
    ...
    const composeFiles = files.filter((f) => f.relativePath === "docker-compose.yml" ||
        f.relativePath === "docker-compose.yaml");
...
function checkDockerPermissions(file, findings) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (/^USER\s+root/i.test(line)) {
            findings.push({
                ...
                description: "The Dockerfile sets USER to root, granting the container full host privileges if escaped.",
        ...
        if (line === "USER 0" || /^USER\s+0\s*$/i.test(line)) {
            findings.push({
                ...
                description: "The Dockerfile explicitly uses UID 0 (root), which is unnecessary for most applications.",
                recommendation: "Use a non-root user or a specific non-zero UID.",
                evidence: { instruction: line },
            });
        }
    }
}
function checkDockerComposePermissions(file, findings) {
    const lines = file.content.split("\n");
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.includes("privileged: true") || line.includes("privileged:true")) {
            findings.push({
                ...
                description: "A service is configured with privileged: true, granting full access to host devices and capabilities.",
                recommendation: "Remove privileged mode and add only specific required capabilities.",
        ...
        if (line.includes("network_mode: host") || line.includes("network_mode:host")) {
            findings.push({
                ...
                description: "A service uses host network mode, bypassing Docker network isolation.",
                recommendation: "Use bridge networking and explicitly expose required ports.",
                evidence: { line: lines[i].trim() },
            });
        }
        if (line.includes("cap_add:") || line.match(/^\s+-\s+cap_add:/i)) {
            findings.push({
                ...
                description: "Docker capabilities are being added beyond the default set.",
                recommendation: "Review each added capability and ensure it is strictly necessary.",
                evidence: { line: lines[i].trim() },
            });
        }
    }
}
function checkEnvFilePermissions(file, findings) {
    const lines = file.content.split("\n");
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i].trim();
        ...
        if (sensitivePatterns.some((p) => p.test(key)) &&
            ...
            findings.push({
                ...
                recommendation: "Remove sensitive values from committed env files. Use .env.example with placeholders instead.",
                evidence: { key, hasValue: value.length > 0 },
            });
        }
    }
}
function checkKubernetesPermissions(file, findings) {
    const lines = file.content.split("\n");
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i].trim();
        if (line.includes("runAsUser: 0") || line.includes("runAsNonRoot: false")) {
            findings.push({
                ...
                description: "The pod security context allows running as root user.",
        ...
        if (line === "privileged: true" || line === "privileged:true") {
            findings.push({
                ...
                description: "Container security context has privileged: true.",
        ...
        if (line.includes("cluster-admin") || line.includes("ClusterRole")) {
            findings.push({
                ...
                description: "RBAC configuration references cluster-admin or ClusterRole, which grants broad permissions.",
                recommendation: "Use namespace-scoped Roles with minimal required permissions.",
                evidence: { line },
            });
        }
    }
